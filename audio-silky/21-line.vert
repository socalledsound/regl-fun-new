...
void main() {
  ...
  float frequencies = analyse(audioTexture, position.x * 0.2 + 0.5);
  #endif
  ...
  #ifdef HAS_VERTEX_SAMPLER
  angleOffset *= mix(1.0, 1.5, frequencies);
  #endif
  ...
  #endif
  computedRadius += turb(angle, 0.1, 6.0, iGlobalTime * 0.005);
  computedRadius += turb(angle, 0.1, 2.5, iGlobalTime * 0.005);
